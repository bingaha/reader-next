...
| 项目 | 说明 |
|------|------|
| 文本模型 Base URL | 模型服务地址，可配合接口路径组成最终请求 URL |
| 文本模型 | 用于整理世界观、角色、关系和地点 |
| 文本模型接口路径 | 默认 `/v1/chat/completions`；可改为兼容网关路径，如 `/v1beta/openai/chat/completions`、`/v1/responses` |
| 文本模型 API Key | 按服务要求填写，可为空 |
| 图片模型 Base URL | 图片模型服务地址 |
| 图片模型 | 用于重绘世界地图 |
| 图片模型接口路径 | 默认 `/v1/images/generations` |
| 图片尺寸 | 支持 `1024x1024`、`1792x1024`、`1024x1792` |
| 使用后端代理调用模型 | 开启后由 Reader Next 后端转发请求，避免浏览器直接跨域调用 |

如果你的模型服务已经提供完整接口地址，可以打开 **完整链接**，让 Base URL 按完整 URL 使用，此时接口路径不会再追加。后端代理不再强制文本请求落到 `/v1/chat/completions`，管理员可在后端配置中改接口路径以接入 Claude、Gemini 或其他兼容网关；但 AI资料文本生成的请求体仍按 Chat Completions/兼容格式组织，原生 Claude/Gemini 协议需要对应网关或后续适配。
